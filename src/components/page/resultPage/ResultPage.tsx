import { useState, useRef, useEffect, UIEvent, useContext, useCallback } from "react";
import { Link, useNavigate } from 'react-router-dom';
import { Icon, Skeleton, Tooltip, Button as MuiButton } from '@mui/material';

import { KeyboardArrowLeft, KeyboardArrowRight, AddCircleOutline, ArrowRight } from "@mui/icons-material";
...
function ResultPage({}:ResultPageProps){
    ...
    useSetElement({
        element:              
            ...
            <div className = 'flex flex-row items-center -translate-y-1'>
                <FocusContextProvider>       
                    {(strings.steps)?.map(({label, icon}: {label: string, icon: string}, index: number)=>{
                        return (
                            <>
        ...
        dep: [ activeSectionIndex, strings.steps ]
    });
    ...
    useEffect(()=>{
        setActiveSectionIndex( activeCheckpointIndex as number );
    }, [ activeCheckpointIndex, setActiveSectionIndex ])

    /*  상단 Stepper의 step 버튼 클릭 핸들러.
    1. step 에 해당하는 div로 스크롤. 2. Stepper 상태 업데이트. */
    const handleClickStepButton = (index: number) => {
        setActiveSectionIndex(index); 
        scrollCheckpoints.current[index].scrollIntoView({behavior: "smooth"});
    }

    /* 페이지 스크롤 시 스크롤 위치 업데이트 */
            ...
            <div className='flex flex-col pt-24 space-y-20 [&>*>*]:pl-16'>
                ...
                <div className="space-y-10">
                        ...
                        <Card className='w-64 h-64 flex flex-col'>
                            {
                                ...
                                <div className='p-4 space-y-8'>                                    
                                    ...
                                    <div>
                                        <h6>{strings.addFriend.giveMyName}</h6>
                                ...
                                <ToggleButton isActive={false} onClick={handleClickAddFriend}>                            
                                    <h4 className=''>{strings.addFriendButton}<AddCircleOutline /></h4>
                                </ToggleButton>                                    
                                </div>
                            }
                        </Card>
                    ...
                    <div className='flex justify-center'>
                            ...
                            >
                                <Button disabled={Object.keys(userList).length <= 1}>{strings.startChemistryButton}<KeyboardArrowRight/></Button>
                            </ConditionalWrapper>
                    </div>
                </div>                
            </div>
...
// const UserListApiLoader = withUserListLoadStatus(ApiLoader)(userName);
// const PlaceGroupApiLoader = withPlaceGroupLoadStatus(ApiLoader);
...
function ResultPageWithApi( props : ResultPageProps ){
    ...
    const [ status2, setStatus2 ] = usePlaceGroupLoadStatus({});
    useLoadStatus({ status, setStatus});
    useLoadStatus({ status: status2, setStatus: setStatus2});
