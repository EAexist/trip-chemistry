...
interface selectedItemContextProps {
  ...
  // setfocusContainer: (focus: boolean) => void,
};
...
function CardCarouselItem({ id, onHoverElement, children }: PropsWithChildren<CardCarouselItemProps>){
  ...
  const { selectedItemId, setSelectedItemId, focusContainer } = useContext(SelectedItemContext); 
  ...
  return(
    <div 
      className={`
        ...
        ${isSelected && !focusContainer && 'shrink-0 opacity-100'}         
